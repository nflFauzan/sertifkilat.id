...
// Marketing copy — ini adalah testimonial kuratif untuk landing page.
// Data ini bukan dari database; ini adalah teks branding yang stabil.
          ...
          >
            <div className="flex gap-0.5 text-amber-400 mb-4">
              {Array.from({ length: 5 }).map((_, j) => (
                <Star key={j} size={15} weight="fill" />
              ))}
            </div>
            ...
            <div className="flex items-center gap-3 pt-4 border-t border-ink-100">
              <div
                className={`w-9 h-9 rounded-full ${t.color} text-white text-xs font-semibold flex items-center justify-center shrink-0`}
              >
                {t.initials}
              </div>
              <div>
